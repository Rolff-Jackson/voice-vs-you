...
function exportWAV(type){
  ...
  for (var channel = 0; channel < numChannels; channel++){
    buffers.push(mergeBuffers(recBuffers[channel],0, recLength));
  }
...
function mergeBuffers(recBuffers, start, recLength){
  ...
  for (var i = 0; i < recBuffers.length; i++){
    if ( offset >= start ) {
      result.set(recBuffers[i], offset2);
      offset2 += recBuffers[i].length;
    }
    else {
